...
pub fn init(size:usize) -> Vec<Vec<f64>>{
    ...
    let mut system: Arc<Mutex<Vec<Vec<f64>>>> = Arc::new(Mutex::new(vec![vec![0.0;size+1];size]));
    for i in 0..size{
        for j in 0..size{
...
*/
pub fn eliminate(mut system: Arc<Vec<Vec<f64>>>, size: usize, num_threads: usize) -> Option<Vec<f64>> {
