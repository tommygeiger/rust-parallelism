...
pub fn init(size:usize) -> [[f64;1025];1024] {
    //init matrix
    let mut rng = rand::thread_rng();
    let mut system: [[f64;1025];1024] = [[0.0;1025];1024];
...
pub fn run(system: [[f64;1025];1024], size: usize, num_threads: u32) -> Vec<f64>{
    //solve
...
pub fn swap_rows(system: &mut [[f64;1025];1024], size: usize, a: usize , b: usize)  {
    let mut temp;
    for i in 0..size+1 {
        temp=system[a][i];
...
pub fn print_matrix(system: &[[f64;1025];1024], size: usize){
    for ip in 0..size{
        for jp in 0..size + 1{
            print!(" {}, ",system[ip][jp]);
        }
...
pub fn eliminate(mut system: [[f64;1025];1024], size: usize) -> Option<Vec<f64>> {
